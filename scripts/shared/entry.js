...
module.exports = __toCommonJS(shared_entry_exports);
...
function normalizeRelativePath(root, filePath) {
  const relativePath = import_node_path.default.relative(
    import_node_path.default.resolve(root),
    import_node_path.default.resolve(filePath)
  );
  if (!relativePath || relativePath === ".." || relativePath.startsWith(`..${import_node_path.default.sep}`) || import_node_path.default.isAbsolute(relativePath)) {
    return null;
  }
...
function splitFrontMatter(raw) {
  ...
  return {
    data: (0, import_yaml.parse)(yamlText) ?? {},
...
function parsePaths(data) {
  ...
  if (!Array.isArray(rawPaths) || !rawPaths.every(
...
async function parseGuidanceFile(options) {
  ...
  let size = 0;
  try {
    size = (await (0, import_promises.stat)(options.filePath)).size;
  } catch (error) {
    return issue(
      options,
      "read-error",
  ...
  let raw = "";
  try {
    raw = await (0, import_promises.readFile)(options.filePath, "utf8");
  } catch (error) {
    return issue(
      options,
      "read-error",
  ...
  return {
    document: {
      ...
      source: options.source,
      root: import_node_path.default.resolve(options.root),
      filePath: import_node_path.default.resolve(options.filePath),
      relativePath,
      paths,
...
// src/core/discover.ts
var DEFAULT_MAX_GUIDANCE_BYTES = 256 * 1024;
function getGuidanceRoots(options) {
  const homeDir = options.homeDir ?? import_node_os.default.homedir();
  return [
    { source: "user", root: import_node_path2.default.join(homeDir, ".codex", "guidance") },
    {
      source: "codex",
      root: import_node_path2.default.join(options.repoRoot, ".codex", "guidance")
    },
    {
      source: "agents",
      root: import_node_path2.default.join(options.repoRoot, ".agents", "guidance")
    },
    { source: "claude", root: import_node_path2.default.join(options.repoRoot, ".claude", "rules") }
  ];
}
...
async function listMarkdownFiles(root) {
  let entries;
  try {
    entries = await (0, import_promises2.readdir)(root, { withFileTypes: true });
  } catch {
    return [];
  }
  const files = [];
  for (const entry of entries) {
    const entryPath = import_node_path2.default.join(root, entry.name);
...
function normalizeTargetPath(targetPath, repoRoot2) {
  ...
  if (import_node_path3.default.isAbsolute(targetPath)) {
    const relativePath = import_node_path3.default.relative(
      import_node_path3.default.resolve(repoRoot2),
      import_node_path3.default.resolve(targetPath)
    );
    const posixRelativePath2 = toPosixPath(relativePath);
    if (posixRelativePath2.length === 0 || isRelativeTraversal(posixRelativePath2) || import_node_path3.default.isAbsolute(relativePath)) {
      return null;
    }
    return posixRelativePath2;
  }
  const posixRelativePath = normalizedInput.replace(/^\.\/+/, "");
  if (posixRelativePath.length === 0 || isRelativeTraversal(posixRelativePath) || import_node_path3.default.posix.isAbsolute(posixRelativePath)) {
    return null;
  }
...
function guidanceMatchesPath(document, normalizedPath) {
  ...
  return document.paths.some(
    (pattern) => minimatch(normalizedPath, pattern, {
      dot: true,
      matchBase: false,
      nocase: false
    })
...
function findMatchingGuidance(options) {
  ...
  return options.documents.filter(
    (document) => guidanceMatchesPath(document, normalizedPath)
  );
}

// src/core/path_extract.ts
var PATH_FIELD_NAMES = /* @__PURE__ */ new Set([
  "path",
  "filePath",
  "filepath",
  "file_path",
...
var PATH_ARRAY_FIELD_NAMES = /* @__PURE__ */ new Set(["paths", "files"]);
var NESTED_INPUT_FIELD_NAMES = /* @__PURE__ */ new Set(["arguments", "params", "input"]);
...
function looksLikeReadWriteEditTool(toolName) {
  ...
  return normalized === "read" || normalized === "write" || normalized === "edit" || normalized === "multiedit" || normalized.includes("read") || normalized.includes("write") || normalized.includes("edit");
}
...
function extractApplyPatchPaths(toolInput) {
  ...
  let match2;
  while ((match2 = headerPattern.exec(patchText)) !== null) {
    addPath(paths, seen, match2[1]);
  }
  return paths;
}
...
// src/core/render.ts
var GLOBAL_HEADER = "Below are global guidance for this session. You must follow them in later actions:";
var PATH_HEADER = "Below are guidance related to the current file. You must follow them in later actions:";
...
function renderLoadedStatus(documents) {
  return sortedDocuments(documents).map((document) => `${document.id} loaded`).join("\n");
}

// src/core/state.ts
var import_promises3 = require("node:fs/promises");
var import_node_path4 = __toESM(require("node:path"));
var DEFAULT_LOCK_TIMEOUT_MS = 250;
var DEFAULT_LOCK_RETRY_MS = 10;
...
function sanitizeSessionId(sessionId) {
  const parts = sessionId.split(/[\\/]+/).map((part) => part.trim()).filter((part) => part.length > 0 && part !== "." && part !== "..");
  const normalized = parts.join("-").replace(/[^A-Za-z0-9._-]+/g, "-").replace(/-{2,}/g, "-").replace(/^[.-]+|[.-]+$/g, "");
  return normalized.length > 0 ? normalized : "session";
}
function resolvePluginDataDir(options) {
  if (options.pluginDataDir !== void 0 && options.pluginDataDir.trim().length > 0) {
    return import_node_path4.default.resolve(options.pluginDataDir);
  }
  const envPluginData = process.env.PLUGIN_DATA;
  if (envPluginData !== void 0 && envPluginData.trim().length > 0) {
    return import_node_path4.default.resolve(envPluginData);
  }
  throw new Error("PLUGIN_DATA is required for codex-guidance session state.");
}
function getStatePaths(options) {
  const sessionId = sanitizeSessionId(options.sessionId);
  const stateDir = import_node_path4.default.join(
  ...
  return {
    sessionId,
    stateDir,
    stateFile: import_node_path4.default.join(stateDir, `${sessionId}.json`),
    lockFile: import_node_path4.default.join(stateDir, `${sessionId}.lock`)
  };
}
function normalizeState(value) {
  ...
  if (typeof payload.loaded !== "object" || payload.loaded === null || Array.isArray(payload.loaded)) {
    return null;
  }
  const loaded = {};
  ...
  if (loaded[currentGeneration] === void 0) {
...
async function readExistingState(stateFile) {
  let raw = "";
  try {
    raw = await (0, import_promises3.readFile)(stateFile, "utf8");
  } catch {
    return defaultState();
  }
  try {
...
async function acquireLock(lockFile, timeoutMs, retryMs) {
  const start = Date.now();
  while (true) {
    try {
      await (0, import_promises3.writeFile)(lockFile, `${process.pid}
`, { flag: "wx" });
      return true;
    } catch {
...
async function releaseLock(lockFile) {
  try {
    await (0, import_promises3.unlink)(lockFile);
  } catch {
  }
}
async function writeStateAtomically(paths, state) {
  await (0, import_promises3.mkdir)(paths.stateDir, { recursive: true });
  const tempFile = import_node_path4.default.join(
    paths.stateDir,
    `${paths.sessionId}.${process.pid}.${Date.now()}.${Math.random().toString(16).slice(2)}.tmp`
  );
  try {
    await (0, import_promises3.writeFile)(tempFile, `${JSON.stringify(state, null, 2)}
`, "utf8");
    await (0, import_promises3.rename)(tempFile, paths.stateFile);
  } catch (error) {
    try {
      await (0, import_promises3.unlink)(tempFile);
    } catch {
    }
    throw error;
  }
}
async function updateSessionState(options, update) {
    ...
    return {
      ...
      loaded: {
        ...
        [generation]: [
          .../* @__PURE__ */ new Set([
            ...loaded,
            ...options.guidanceIds.map((id) => id.trim()).filter((id) => id.length > 0)
          ])
        ]
      }
    };
...
function isEditTool(toolName) {
  ...
  const normalized = toolName.toLowerCase();
  return normalized === "write" || normalized === "edit" || normalized === "multiedit" || normalized === "apply_patch" || normalized.includes("write") || normalized.includes("edit");
}
...
async function runCli(handler) {
  try {
    const rawInput = (0, import_node_fs.readFileSync)(0, "utf8");
...
async function discoverForHook(input, context = {}) {
  ...
  return (await discoverGuidance({
    repoRoot: repoRoot(input, context),
    ...homeDir === void 0 || homeDir.trim().length === 0 ? {} : { homeDir }
  })).documents;
}
...
function stateOptions(input, context = {}) {
  ...
  return pluginDataDir === void 0 || pluginDataDir.trim().length === 0 ? { sessionId: input.sessionId } : { sessionId: input.sessionId, pluginDataDir };
}
function stateUpdateOptions(input, context = {}) {
  ...
  return {
    ...baseOptions,
    ...lockTimeoutMs === void 0 ? {} : { lockTimeoutMs },
    ...lockRetryMs === void 0 ? {} : { lockRetryMs }
  };
}
...
// src/hooks/pre_tool_use.ts
var RETRY_REASON = "Codex Guidance loaded matching guidance. Retry the edit after applying the loaded guidance.";
